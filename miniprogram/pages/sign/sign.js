...
Page({
  ...
  submit(e) {
    ...
    console.log(dd);
    if (!dd.nickName || !dd.phone || !dd.passWord || !dd.con_passWord || !dd.invite_code) {
      wx.showToast({
        title: '请输入完整信息',
        icon: 'none'
      })
      ...
      db.collection("admin").where({
        ...
        .then(res => {
          if (res.data.length) //已存在
          {
            ...
            this.add_user(dd.nickName, dd.phone, dd.passWord, dd.invite_code);
          }
        })
        .catch(err => {
          wx.showToast({
  ...
  // 添加用户
  add_user(nickName, phone, passWord, invite_code) {
    let that = this
  ...
  onChooseAvatar(e) {
    ...
    wx.cloud.uploadFile({
      cloudPath: "admins.images/" + time, //文件名
